...
func (t *Tiler) countTileTypesIn8Around(x, y int, types ...uint8) int {
	count := 0
	for i := x - 1; i <= x+1; i++ {
		for j := y - 1; j <= y+1; j++ {
			if (i != x || j != y) && t.areCoordsValid(i, j) {
...
func (t *Tiler) countTileTypesInPlusAround(x, y int, types ...uint8) int {
	count := 0
	for i := -1; i <= 1; i++ {
		for j := -1; j <= 1; j++ {
			if (i*j == 0) && (i != 0 || j != 0) && t.areCoordsValid(x+i, y+j) {
				for _, typ := range types {
